...
void abms_init_loop(void)
{
...
#if defined(ABMS_RT_DEBUG)
	sprintf(lcmd,"exec %s%s",ABMS_BINPATH,ACMD_SH);
	abms_system(lcmd);
#endif /* defined(ABMS_RT_DEBUG) */
	...
	sprintf(lcmd,"%s%s -l %s%s -f %s 38400 %s",ABMS_BINPATH,ACMD_GETTY,ABMS_BINPATH,ACMD_LOGIN,ABMS_ISSUENET,ABMS_SECTTY);
	abms_system(lcmd);
