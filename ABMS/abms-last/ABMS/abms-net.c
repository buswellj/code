...
extern abms_netdev_t abms_netdev_table;
...
{
	int rc = RC_OK;
	abms_netdev_t *netptr = &abms_netdev_table;

	abms_mgmtfound = 0;
	memset(netptr,'\0',sizeof(abms_netdev_table));
...
u8 abms_net_chkmac(char *devname)
{
	...
	/* scan interfaces for matching mac */
		...
		z = abms_netdev_table.nxt_idx;
		abms_netdev_table.nxt_idx++;

		abms_strlcpy(abms_netdev_table.phy[z].devname,devname,(strlen(devname) + 1));

		for (macc = 0; macc < 6; macc++) {
#if defined(ABMS_DEBUG_ANNOYING)
		    printf("%02x ",mac->ether_addr_octet[macc]);
#endif
		    abms_netdev_table.phy[z].mac[macc] = mac->ether_addr_octet[macc];
		}

		    printf("\n");

		if (macm == 6) {
		    abms_netdev_table.phy[z].mgmt = 1;
...
{
	u8 i,e,mac = 0, me = 0xff, mi = 0xff;
	

	if (!abms_prodip_table.nxt_idx) {
	    return;				/* no production ips, stick with default */
	}

	for (i = 0; i < abms_prodip_table.nxt_idx; i++) {
	    if (abms_netdev_table.nxt_idx) {
	        for (e = 0; e < abms_netdev_table.nxt_idx; e++) {
		    for (mac = 0; mac < 6; mac++) {
			if (abms_netdev_table.phy[e].mac[mac] == abms_prodip_table.ipnet[i].mac[mac]) {
			   continue;
			} else {
			   mac = 6;
			}
